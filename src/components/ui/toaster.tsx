...
function ToastItem({ toast }: { toast: Toast }) {
  ...
  const body = (
    <>
      ...
      {clickable && <ChevronRight className="h-4 w-4 flex-shrink-0 opacity-60" />}
    </>
  );

  const className = `flex w-full items-center gap-2.5 rounded-lg border px-4 py-3 text-sm font-medium shadow-lg ${
  ...
  if (toast.href) {
    return (
      <motion.div
        ...
        className="w-full"
      >
        <Link href={toast.href} className={className} onClick={toast.onClick}>
          {body}
        </Link>
      </motion.div>
    );
